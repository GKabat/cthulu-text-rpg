...
| `data/story.json` | cała fabuła: wszystkie sceny i wybory | scenariusz |
| `data/config.json` | ustawienia startowe (tytuł, startowe HP/Sanity) | metryczka gry |
| `tiles/` | grafiki (ramka okna, kość, obrazki scen) | dekoracje |
...
**Ramka leży na wierzchu.** Grafika ramki (`tiles/frame.png`) ma **przezroczysty
...
„KONTYNUUJ" w menu. Zapis to jeden plik `data/save.json`. Ponieważ stan gry to
